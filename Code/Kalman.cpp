...
Kalman::Kalman() {
    ...
    // set the values for the process and the measurement variances
    V1_angle   = 0.001f;    // defaults: 0.001 - try: 0.001
    V1_bias    = 0.001f;    // defautls: 0.003 - try: 0.003
    V2_measure = 2.90f;     // defaults: 0.030 - try: 9.900

    // initialization of the angle, the bias and the time interval
    angle = 0.0f;
    bias = 0.0f;
    prevDeltaTime = 0.0f;
...
float Kalman::getAngle(float accAngle, float gyroRate, float deltaTime) {
    ...
    // P(t|t-1) = F(t-1)P(t-1|t-1)F(t-1)' + V1
    P[0][0] += prevDeltaTime * (prevDeltaTime*P[1][1] - P[0][1] - P[1][0] + V1_angle);
    P[0][1] -= prevDeltaTime * P[1][1];
    P[1][0] -= prevDeltaTime * P[1][1];
    P[1][1] += V1_bias * prevDeltaTime;
